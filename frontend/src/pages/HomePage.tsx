...
export default function HomePage({ mode, onToggleTheme }: HomePageProps) {
  ...
  const loadDashboards = async () => {
    try {
      ...
      // Se tem dashboardId na URL, seleciona ele
      if (dashboardId) {
        const dash = data.find((d: any) => d.id === dashboardId);
        if (dash) {
          setSelectedDashboard(dash);
        }
      } else if (data.length > 0 && !selectedDashboard) {
        // Seleciona o primeiro dashboard por padrão
  ...
  const handleCreateDashboard = async () => {
    try {
      const newDash = await dashboardService.create(title, description);
    ...
    <Box sx={{ minHeight: '100vh', bgcolor: 'background.default' }}>
      ...
      {!selectedDashboard ? (
        ...
        <Box>
          ...
          <DashboardFinancial dashboardId={selectedDashboard.id} />
        </Box>
      )}
